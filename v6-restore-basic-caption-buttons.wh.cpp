...
// Re-stacks button visuals to the top of the CTopLevelWindow Z-order
...
long CTopLevelWindow_UpdateNCAreaPositionsAndSizes_Hook(void *pThis) {
    ...
    // Normal (Unmaximized / Restored) Windows:
    ...
    // Select offsets based on window state
    ...
    if (hasIcon) {
        ...
        if (*(long long *)(someData + 136) || (*((long *)pThis + 148) & 0x10000) == 0) {
            // Lock icon visual size to 16x16
            width = 16;
            height = 16;
        }
        else {
        ...
        // Force point sampling (Nearest Neighbor) to keep raw pixelated look
        if (CVisual_SetBitmapScalingMode_Original) {
            CVisual_SetBitmapScalingMode_Original(iconData, 2); // 2 = NearestNeighbor
        }
        // Force Aliased edge rendering (no smooth anti-aliased edge blending)
        if (CVisual_SetEdgeMode_Original) {
            CVisual_SetEdgeMode_Original(iconData, 1); // 1 = Aliased
        }

        int insetTopIcon;
        if (isMaximized) {
            insetTopIcon = (*((DWORD *)pThis + 151) - *(DWORD *)(*((long long *)pThis + 66) + 124)) / 2;
        } else {
            insetTopIcon = borderSizeArray[2] + (*((DWORD *)pThis + 151) - *(DWORD *)(*((long long *)pThis + 66) + 124) - borderSizeArray[2]) / 2;
        }

        CVisual_SetInsetFromParentTop_Original(iconData, insetTopIcon + iconOffsetTop);
        CVisual_SetInsetFromParentLeft_Original(iconData, insetLeft + iconOffsetLeft);
    ...
    if (hasText) {
        ...
        int textTopInset;
        if (isMaximized) {
            textTopInset = titleOffsetTop;
        } else {
            textTopInset = borderSizeArray[2] + titleOffsetTop;
        }

        CVisual_SetInsetFromParentTop_Original(textData, textTopInset);
        CVisual_SetInsetFromParentLeft_Original(textData, insetLeft + titleOffsetLeft);
...
// Callback function to force DWM to update all visible windows immediately
...
BOOL Wh_ModInit(void) {
    ...
    // Force DWM to update all existing window caption buttons immediately on load
